...
ComPtr<ID2D1Image> ColorMatrixEffectRenderer::render(
	...
	ColorMatrixEffect const& effect) const {
	ComPtr<ID2D1Effect> colorMatrixEffect;
	HRESULT result = context.getD2dContext()->getDeviceContext()->CreateEffect(CLSID_D2D1ColorMatrix, &colorMatrixEffect);
	if (FAILED(result) || !colorMatrixEffect) {
		return nullptr;
	}

	colorMatrixEffect->SetInput(0, sourceImage.Get());
	colorMatrixEffect->SetValue(D2D1_COLORMATRIX_PROP_COLOR_MATRIX, toD2dColorMatrix(effect.colorMatrix));
	colorMatrixEffect->SetValue(D2D1_COLORMATRIX_PROP_ALPHA_MODE, mapAlphaMode(effect.alphaMode));
	colorMatrixEffect->SetValue(D2D1_COLORMATRIX_PROP_CLAMP_OUTPUT, static_cast<BOOL>(effect.clampOutput));
